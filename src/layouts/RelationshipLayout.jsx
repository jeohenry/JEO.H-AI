...
const RelationshipLayout = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`/relationship/user/${userId}`, {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`ws://localhost:8000/ws/notify/${userId}`);
    ws.onmessage = (e) => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gradient-to-br from-pink-100 to-rose-100 dark:from-gray-900 dark:to-gray-800 text-gray-800 dark:text-white">
      {/* Sidebar */}
      <aside className={`fixed md:static z-40 h-full w-64 transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 transition-transform bg-gradient-to-b from-rose-600 to-pink-500 dark:from-gray-800 dark:to-gray-700`}>
        ...
        <div className="flex flex-col items-center text-white mt-4">
          <div className="w-20 h-20 rounded-full overflow-hidden shadow border-4 border-rose-300">
            {user.avatar ? <img src={user.avatar} alt="Avatar" className="w-full h-full object-cover" /> : <FaUserCircle className="text-6xl text-white" />}
          </div>
      ...
      <div className="flex-1 flex flex-col">
        <header className="flex justify-between items-center px-5 py-3 bg-white dark:bg-gray-900 shadow sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold text-rose-700 dark:text-pink-300">Relationship AI</h1>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => navigate("/relationship/notifications")} className="relative">
            ...
            <div className="relative group">
              <button className="flex items-center gap-2">
                {user.avatar ? <img src={user.avatar} alt="User" className="w-8 h-8 rounded-full object-cover border-2 border-rose-300" /> : <FaUserCircle className="text-2xl" />}
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-md shadow-md hidden group-hover:block">
                <button onClick={() => navigate("/relationship/profile")} className="block w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-700">Edit Profile</button>
                <button onClick={() => navigate("/relationship/upload")} className="block w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-700">Upload Picture</button>
                <button onClick={logout} className="flex items-center gap-2 text-red-600 dark:text-red-400 w-full px-4 py-2 hover:bg-red-100 dark:hover:bg-red-800">
                  <FaSignOutAlt /> Logout
                </button>
              </div>
            </div>
